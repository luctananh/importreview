...
export default function ProductTable() {
  ...
  function handleUpload(url, error) {
    ...
    } else if (!validateImage(url)) {
      setErrorMessage("Please upload the correct image format");
      setImageUrl("");
    } else {
  ...
  function handleInputChange(event) {
    ...
    if (!validateImage(url)) {
      setErrorMessage(
        "Invalid image format. Please enter a valid URL ending with .jpg, .jpeg, .png, or .gif."
      );
      setImageUrl(""); // Reset nếu không hợp lệ
    } else {
      setImageUrl(url);
      setErrorMessage(""); // Reset lỗi nếu hợp lệ
    }
  }
  async function handleSubmit(event) {
    setLoading(true);
    if (!validateImage(imageUrl)) {
      setErrorMessage("Please upload a valid image before submitting.");
      setLoading(false);
      return;
    }
